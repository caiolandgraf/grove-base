...
func InitSessionManagerFromEnv(redisPool *redis.Pool) *scs.SessionManager {
	sessionManager := InitSessionManager(redisPool)

	// Override with env vars
	if getEnv("ENVIRONMENT", "development") == "production" {
		sessionManager.Cookie.Secure = true
		sessionManager.Cookie.Domain = getEnv("COOKIE_DOMAIN", "")
	}
