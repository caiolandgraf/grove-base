...
- **Go** 1.25+
- **Docker** & **Docker Compose** (for PostgreSQL, Redis, Jaeger)
...
| `OTEL_EXPLOERER_OTLP_ENDPOINT` | `http://localhost:4318`     | OTLP HTTP collector endpoint   |
...
This starts **PostgreSQL**, **Redis**, and **Jaeger**.
...
The project uses Go's standard library **`log/slog`** with a JSON handler for structured logging.
...
{"time":"2025-07-17T10:00:00Z","level":"INFO","msg":"OpenTelemetry initialized","service":"go-project-base","endpoint":"http://localhost:4318"}
```
...
The project ships with full **OpenTelemetry** tracing:
...
All traces are exported via OTLP HTTP to the configured collector endpoint.
